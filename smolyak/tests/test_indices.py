...
class TestSparseIndex(unittest.TestCase):
    ...
    def test_rectangle(self):
        self.assertCountEqual(
            indices.rectangle(L=[4, 4], n=2),
            [mi for mi in cartesian_product([range(4)] * 2)],
        )
        self.assertCountEqual(
            indices.rectangle(L=2, n=2),
            [MultiIndex(), MultiIndex((1, 0)), MultiIndex((0, 1)), MultiIndex((1, 1))],
        )

    def test_simplex(self):
        self.assertCountEqual(
            indices.simplex(L=1, n=2),
            [MultiIndex(), MultiIndex((1, 0)), MultiIndex((0, 1))],
        )
    ...
    def test_cartesian_product(self):
        ...
        self.assertEqual(
            [mi.full_tuple() for mi in TT], [(1, 3, 2), (1, 4, 2), (2, 3, 2), (2, 4, 2)]
        )
        TT = cartesian_product(T, (1, 3, 4))
        self.assertEqual(
            [mi.full_tuple() for mi in TT],
            [(0, 1, 0, 3, 2), (0, 1, 0, 4, 2), (0, 2, 0, 3, 2), (0, 2, 0, 4, 2)],
        )

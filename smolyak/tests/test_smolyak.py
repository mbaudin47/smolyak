...
class TestSparseApproximation(unittest.TestCase):
    def test_dimensionadaptivity(self):
        ...
        decomp = Decomposition(
            func=func, n=inf, next_dims=lambda dim: [dim + 1] if dim < ndim - 1 else []
        )
    ...
    def test_bundled(self):
        ...
        def func(mis):
            value = sum(
                [np.exp(-3 * mi[0]) * np.exp(-mi[1]) * np.exp(-mi[2]) for mi in mis]
            )
            return value + snooze(
                sum([2 ** mi[0] * 2 ** mi[1] * 2 ** mi[2] for mi in mis])
            )
        ...
        decomp = Decomposition(
            ...
            n=3,
            bundled=True,
            bundled_dims=bundled_dims,
            work_multipliers=[2, 2, 2],
            contribution_multipliers=np.exp([-3, -1, -1]),
        )
        SA = SparseApproximation(decomp)
        ...
        self.assertAlmostEqual(
            T,
            1 / (1 - np.exp(-3)) * 1 / (1 - np.exp(-1)) * 1 / (1 - np.exp(-1)),
            places=2,
            msg="Limit incorrect",
        )
    ...
    def test_notbundled(self):
        def func(i):
            value = 3.0 ** (-i[0]) * 5.0 ** (-2 * i[1]) * 7.0 ** (-3 * i[2])
            return value + snooze(2 ** i[0] * 2 ** i[1] * 2 ** i[2])

        contribution_factor = (
            lambda i: 3.0 ** (-float(i[0]))
            * 5.0 ** (-2 * float(i[1]))
            * 7.0 ** (-3 * float(i[2]))
        )
        decomp = Decomposition(
            func=func, n=3, contribution_function=contribution_factor
        )
        SA = SparseApproximation(decomp)
        print_runtime(SA.expand_adaptive)(N=75, T=2)
        self.assertAlmostEqual(
            SA.get_approximation(), 3.0 / 2 * 25.0 / 24 * 343.0 / 342, places=2
        )
    ...
    def test_notbundled2(self):
        ...
        self.assertAlmostEqual(
            SA.get_approximation(), 3.0 / 2 * 25.0 / 24 * 343.0 / 342, places=3
        )
        self.assertAlmostEqual(SA.get_runtime_multiplier(0), 2, delta=0.2)

...
class MultiIndex:
    ...
    def __lt__(self, other):
        ...
        if not (d1 or d2):
            ...
            dim_max = max(dims)
            return np.all(
                np.less(self.full_tuple(dim_max + 1), other.full_tuple(dim_max + 1))
            )
    ...
    def __le__(self, other):
        ...
        if not (d1 or d2):
            ...
            return np.all(
                np.less_equal(
                    self.full_tuple(dim_max + 1), other.full_tuple(dim_max + 1)
                )
            )
...
class MISet:
    ...
    def add(self, mi):
        ...
        if not mi in self.mis:
            ...
            new_candidates = (
                mi + sign * kronecker(dim)
                for dim, sign in itertools.product(self.active_dims, (-1, 1))
            )
            self.candidates.update(
                mi
                for mi in new_candidates
                if (mi not in self.mis and mi >= MultiIndex())
            )
    ...
    def add_dimensions(self, dims):
        for dim in dims:
            if dim in self.active_dims:
                raise ValueError(
                    "{} was already an active dimension of downward closed set".format(
                        dim
                    )
                )
            else:
    ...
    def is_admissible(self, mi):
        ...
        """
        return (mi == MultiIndex()) or any(
            mi + sign * kronecker(dim) in self.mis
            for dim, sign in itertools.product(self.active_dims, (-1, 1))
        )
...
def get_bundle(multi_index, multi_indices, is_bundled):
    ...
    """
    return [mi for mi in multi_indices if multi_index.equal_mod(mi, is_bundled)]
...
def pyramid(L, Gamma_1, Beta_1, Gamma_2, Beta_2, n):
    def admissible(mi):
        return (mi.leftshift() == MultiIndex() and (Gamma_1 + Beta_1) * mi[0] <= L) or (
            mi.leftshift() != MultiIndex()
            and (Gamma_2 + Beta_2) * max([v for __, v in mi.leftshift()])
            + (Gamma_1 + Beta_1) * mi[0]
            <= L
        )
...
def cartesian_product(entries, dims=None):
    ...
    """
    return [
        MultiIndex(zip(dims or range(len(entries)), t), sparse=True)
        for t in itertools.product(*entries)
    ]
...
def tensor_product(sets, ns):
    ...
    return [
        sum(mi.shifted(shifts[k]) for k, mi in enumerate(mis))
        for mis in itertools.product(*sets)
    ]
...
class MixedDifferences:
    ...
    def __init__(
        self, f, store_output=True, zipped=True, c_var=None, reparametrization=False
    ):
        r"""
        ...
        if not self.zipped:
            ...
            if self.reparametrization:
                self.f = lambda mi: f(
                    *MultiIndex(
                        [2 ** v for v in mi.full_tuple(c_dim=self.c_var)]
                    ).full_tuple(c_dim=self.c_var)
                )
            else:
...
def combination_rule(mis):
    ...
    for mi in mis:
        ...
        for down in cartesian_product([[0, 1]] * len(dims), dims):
            mi_neighbor = mi - down
            coefficients[mi_neighbor] = coefficients[mi_neighbor] + (-1) ** len(
                down.active_dims()
            )
    output = {}

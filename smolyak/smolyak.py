...
from applications.polynomials import PolynomialApproximator
    ...
    def __init__(
        ...
        if Passed(multipliers):
            ...
            func = lambda mi: np.prod(
                [self.multipliers[dim] ** mi[dim] for dim in self.multipliers]
            )
            if self.bundled:
...
class _Decomposition:
    ...
    def __init__(
        ...
        contribution_multipliers: Dict(
            value_spec=InInterval(l=0, lo=True, r=1, ro=True), lenience=2
        ),
        ...
        if isinstance(Delta, PolynomialApproximator):
            if Passed(n) or returns_work:
                raise ValueError(
                    "Do not specify `n` or `returns_work` to SparseApproximator for polynomial approximation"
                )
            self.Delta = Delta.update_approximation
            self.n = Delta.n_acc + Delta.n
            _work_function = WorkFunction(
                func=Delta.estimated_work,
                dims=lambda n: (True if Passed(work_function) else n >= Delta.n_acc),
                bundled=True,
            )
            ...
            if isinstance(structure, str):
                if structure.lower() == "td":
                    structure = lambda mi: [
                        ...
                        + mi2.shifted(Delta.n_acc)
                        for mi2 in indices.simplex(
                            n=Delta.n, L=mi.mod(lambda n: n < Delta.n_acc).sum()
                        )
                    ]
                    ...
                    structure = lambda mi: [
                        mi.restrict(lambda n: n < Delta.n_acc)
                        + mi2.shifted(Delta.n_acc)
                        for mi2 in indices.rectangle(
                            n=Delta.n, L=max(mi.mod(lambda n: n < Delta.n_acc))
                        )
                    ]
                            ...
                            ret = [
                                ...
                                for mi2 in [
                                    ...
                                    for perm in multiset_permutations(
                                        mi.mod(lambda n: n < Delta.n_acc).full_tuple()
                                    )
                                ]
                            ]
                            return ret
    ...
    def _set_work(self, work_multipliers, work_function):
        if work_multipliers:
            self.work_function = WorkFunction(
                multipliers=work_multipliers, n=self.n, bundled=self.bundled
            )
        elif work_function:
            if work_function.bundled == self.bundled:
                ...
                raise ValueError(
                    "Work function cannot act on bundles when computations don't"
                )
            elif self.bundled and not work_function.bundled:
                self.work_function = WorkFunction(
                    func=lambda mis: sum(work_function(mi) for mi in mis),
                    dims=work_function.dims,
                    bundled=True,
                )
        else:
            self.work_function = WorkFunction(
                func=lambda _: 1, dims=lambda dim: False, bundled=self.bundled
            )
        if self.bundled != self.work_function.bundled:
            raise ValueError(
                "If computations are bundled, work function must act on bundles as well (and conversely)"
            )

    def _set_contribution(self, contribution_multipliers, contribution_function):
        if contribution_multipliers:
            self.contribution_function = ContributionFunction(
                multipliers=contribution_multipliers, n=self.n, bundled=False
            )
        elif contribution_function:
            self.contribution_function = contribution_function
        else:
            self.contribution_function = ContributionFunction(
                func=lambda _: 1, dims=lambda dim: False, bundled=False
            )
    ...
    def _set_next_dims(self, next_dims):
        if (not next_dims) and self.n == Inf:
            self.next_dims = (
                lambda dim: [dim + 1] if dim + 1 not in self.init_dims else []
            )
        else:
...
class SparseApproximator:
    ...
    def update_approximation(self, mode=None, indices=None, T=None, L=None):
        ...
        if mode == "indices":
            ...
            if (
                self.decomposition.bundled
                and not self.decomposition.returns_contributions
            ):
                raise ValueError(
                    "Cannot run adaptively when decomposition.bundled but not decomposition.returns_contributions"
                )
            if Passed(L):
                it = range(L)
            else:
            ...
            if np.isinf(self.decomposition.n):
                raise ValueError(
                    "Cannot use continuation for infinite-dimensional problems"
                )
            it = itertools.count()
            ...
            work_exponents = lambda: [
                ...
                if (
                    self.data.work_model_estimator
            ...
            contribution_exponents = lambda: [
                ...
                if self.data.contribution_estimator.ratios[i]
                else 1
                for i in range(n)
            ]
        else:
        ...
        for l in it:
            ...
            if mode == "apriori":
                ...
                scale = min(
                    work_exponents() + contribution_exponents() for dim in range(n)
                )
                mis_update = indices.simplex(
                    L=l,
                    weights=(work_exponents() + contribution_exponents()) / scale,
                    n=n,
                )
            elif mode == "indices":
                mis_update = indices
            self._extend(
                mis_update
            )  # check whether this rbreaks when mis_udpate is a subset of current mis
    ...
    def _extend(self, mis):
        ...
        for temp in it:
            ...
            if self.decomposition.bundled:
                ...
                mis_update = [temp]
            if (
                not self.decomposition.stores_approximation
                and not self.decomposition.Delta
            ):  # Dry run
                return
            if self.decomposition.bundled:
                external_work_factor = self.decomposition.work_function(
                    mis_update
                )  # Want to be able to provide work function with whole bundle without checking what is actually new and then asking each of those separately, thats why it is required that work_function covers all bundled dimensions
            else:
            ...
            if self.decomposition.stores_approximation:
                argument = (
                    self.data.mis.mis
                )  # provide full set, leave it to external to reuse computations or not
            else:
            ...
            n_arg = sum(
                map(
                    ...
                    [
                        ...
                        self.decomposition.returns_contributions,
                    ],
                )
            )  # Possible outputs: Decomposition term, work, contribution
            if n_arg == 1 and not isinstance(
                output, tuple
            ):  # Allow user to not return tuples if not necessary
                output = [output]
            ...
            if (
                self.decomposition.returns_work
                and self.decomposition.returns_contributions
            ):  # Remaining 2 outputs
                ...
                not self.decomposition.returns_work
                and self.decomposition.returns_contributions
            ):  # Remaining 1 output
                contribution = output[0]
            if (
                self.decomposition.returns_contributions
            ):  # User decides what contribution means
                if not self.decomposition.bundled_dims and not Dict.valid(
                    contribution
                ):  # Allow user to not return dictionary if not_bundled (which means user is only passed single multi-index)
                    contribution = {mi_update: contribution}
            elif (
                not self.decomposition.stores_approximation
            ):  # If approximation is stored here instead of by user, try to figure out contribution
                try:
                    if (
                        self.decomposition.bundled_dims
                    ):  # If approximation is grouped into bundles, norm function must be able to divide contribution into single multi-indices
                    ...
                    try:
                        if (
                            self.decomposition.bundled_dims
                        ):  # TODO: Does it make sense to assign each term in bundle the same contribution?
                            contribution = {
                                mi: np.linalg.norm(object_slice) for mi in mis_update
                            }
    ...
    def get_runtime_multiplier(self, dim):
        if self.decomposition.returns_work:
            raise ValueError(
                "Since decomposition provides abstract work model, no runtime estimates are kept. Try `get_work_multiplier` instead."
            )
    ...
    def plot_indices(
        ...
        weights: In(
            ...
            "work_model",
            "runtime",
            "contribution/work_model",
            "contribution/runtime",
            False,
        ) = False,
        percentiles: Positive & Integer = 4,
    ):
        """
        ...
        if not weights:
            ...
            weight_dict = {
                mi: self.data.contributions[mi] / self.data.work_models[mi]
                for mi in self.get_indices()
            }
        elif weights == "contribution/runtime":
            weight_dict = {
                mi: self.data.contributions[mi] / self.data.runtimes[mi]
                for mi in self.get_indices()
            }
        else:
            raise ValueError("Cannot use weights {}".format(weights))
        plots.plot_indices(
            mis=self.get_indices(), dims=dims, weights=weight_dict, groups=percentiles
        )


class _Estimator:
    def __init__(
        self, dims_ignore, exponent_max, exponent_min, init_exponents=None, name=""
    ):
        self.quantities = {}
        ...
        self.fallback_exponents = DefaultDict(
            init_exponents
        )  # USED AS PRIOR IN EXPONENT ESTIMATION AND AS INITIAL GUESS OF EXPONENT WHEN NO DATA AVAILABLE AT ALL
    ...
    def _update_exponents(self):
        for dim in self.ratios:
            ...
            estimate = max(
                min(np.median(ratios), np.exp(self.exponent_max)),
                np.exp(self.exponent_min),
            )
            c = len(ratios)
            self.exponents[dim] = (
                self.fallback_exponents[dim] + c * np.log(estimate)
            ) / (c + 1.0)
...
class _Data:
    def __init__(self, decomposition):
        ...
        self.runtimes = MultiIndexDict(
            lambda dim: self.decomposition.bundled
            and self.decomposition.bundled_dims(dim)
        )
        ...
        self.mis.structure_constraints |= set(
            MultiIndex(((i, 1),), sparse=True) for i in self.decomposition.init_dims
        )
    ...
    def next_best_mi(self):
        ...
        for mi in mis.structure_constraints.copy():
            ...
            if self.decomposition.structure:
                mis.structure_constraints |= set(
                    self.decomposition.structure(mi_update)
                )
        return mi_update
    ...
    def apriori_indices(self, L):
        ...
        try:
            mis = get_admissible_indices(
                lambda mi: admissible(mi), self.decomposition.n
            )
        except KeyError:
    ...
    def update_estimates(
        ...
        if self.decomposition.returns_work:  # external_work_factor refers to work_model
            if external_work_factor > 0:
                self.work_model_estimator[mi_update] = (
                    work_model / external_work_factor
                )  # Here lies reason why work factor must be bundled if decomposition is bundled: `work_model` does not distinguish different contributions to work (its for the entire mi_update-bundle), so external_work_factor must also make its predictions for entires bundles
        ...
        try:
            for mi in contribution:
                self.contribution_estimator[mi] = contribution[
                    mi
                ] / self.decomposition.contribution_function(
                    mi
                )  # Here lies reason why the reasoning above does not apply to contributions: they can always be split up among all the multi-indices in a bundle (this is implicit in the assumption that the used provided contributions are separate for each mi in mi_update)
    ...
    def profit_estimate(self, mi):
        contribution = self.contribution_estimator(
            mi
        ) * self.decomposition.contribution_function(mi)
        if self.decomposition.bundled:
            mi_to_bundle = lambda mi: get_bundle(
                mi, self.mis, self.decomposition.bundled_dims
            ) + [mi]
        else:
            mi_to_bundle = lambda mi: mi
        if self.decomposition.returns_work:
            work = self.work_model_estimator(mi) * self.decomposition.work_function(
                mi_to_bundle(mi)
            )
            return contribution / work
        else:
            runtime = self.runtime_estimator(mi) * self.decomposition.work_function(
                mi_to_bundle(mi)
            )
        ...
        for mi in mis_update:
            if mi.is_kronecker() and not mi in self.mis:
                ...
                for dim in dims_new:
                    ...
                    if self.decomposition.returns_work:
                        self.work_model_estimator.set_fallback_exponent(
                            dim, self.work_model_estimator.exponents[dim_trigger]
                        )
                    else:
                        self.runtime_estimator.set_fallback_exponent(
                            dim, self.runtime_estimator.exponents[dim_trigger]
                        )
                    if not self.decomposition.kronecker_exponents:
                        self.contribution_estimator.set_fallback_exponent(
                            dim, self.contribution_estimator.exponents[dim_trigger]
                        )

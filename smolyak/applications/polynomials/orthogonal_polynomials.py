...
def evaluate_orthonormal_polynomials(
    X, max_degree, measure, interval=(0, 1), derivative=0
):
    r"""
    ...
    if derivative > 1 and measure not in ["h", "t"]:
        raise ValueError(
            "Second derivative only implemented for Taylor and Hermite polynomials"
        )
    if measure in ["u", "c"]:
        Xtilde = (X - (interval[1] + interval[0]) / 2.0) / (
            (interval[1] - interval[0]) / 2.0
        )
        if measure == "u":
            ...
            if derivative > 0:
                for _ in range(derivative):
                    ...
                    for j in range(2, max_degree + 1):
                        y1[:, j] = np.sqrt(2 * j + 1) * (
                            (2 * j - 1) * y[:, j - 1] / np.sqrt(2 * j - 1)
                            + y1[:, j - 2] / np.sqrt(2 * j - 3)
                        )
                    y = y1
        ...
        if derivative > 0:
            for _ in range(derivative):
                y = np.concatenate(
                    [np.zeros([X.shape[0], 1]), np.arange(1, y.shape[1]) * y[:, :-1]],
                    axis=1,
                )
        return y
...
def chebyshev_polynomials(X, N):
    ...
    deg = N - 1
    orthonormalizer = np.concatenate(
        (np.array([1]).reshape(1, 1), np.sqrt(2) * np.ones((1, deg))), axis=1
    )
    if deg < 1:
...
def legendre_polynomials(X, N):
    ...
    if deg < 1:
        ...
        for n in range(1, deg):
            out[:, n + 1] = (
                1.0 / (n + 1) * ((2 * n + 1) * X * out[:, n] - n * out[:, n - 1])
            )
    return out * orthonormalizer

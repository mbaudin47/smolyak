...
import unittest
import numpy as np
from smolyak.applications.polynomials.weighted_polynomial_approximator import (
    WeightedPolynomialApproximator,
)
from smolyak.indices import cartesian_product
from smolyak import indices

from smolyak.applications.polynomials.probability_spaces import (
...
class TestWeightedPolynomialApproximator(unittest.TestCase):
    ...
    def foo(self, approximation_type):
        ...
        MIWPA = WeightedPolynomialApproximator(
            function=function, n=n_acc, polynomial_space=poly_space
        )
        if approximation_type == "expand_continuation":
            decomp = Decomposition(
                func=MIWPA.update_approximation,
                n=n_acc + n_par,
                bundled=True,
                bundled_dims=lambda dim: dim >= n_acc,
                stores_approximation=True,
                returns_work=True,
                returns_contributions=True,
                reset=MIWPA.reset,
            )
            ...
            decomp = Decomposition(
                ...
                work_function=WorkFunction(
                    func=MIWPA.estimated_work,
                    dims=lambda dim: dim >= n_acc,
                    bundled=True,
                ),
                stores_approximation=True,
                reset=MIWPA.reset,

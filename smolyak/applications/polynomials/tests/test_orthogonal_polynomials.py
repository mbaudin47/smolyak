...
"""
import unittest
from smolyak.applications.polynomials.orthogonal_polynomials import (
...
class TestOrthogonalPolynomials(unittest.TestCase):
    def test_legendre(self):
        ...
        D = evaluate_orthonormal_polynomials(
            X, degree_max, "u", interval=(a, b), derivative=1
        )
        for j, (y, d) in islice(enumerate(zip(Y.T, D.T)), 10):
            self.assertAlmostEqual(
                y[-1], y[0] + np.sum(d) * (b - a) / cEvaluations, delta=0.1
            )
    ...
    def test_hermite(self):
        ...
        scalarproduct_10_20 = (
            Y[:, 10].transpose().dot(Y[:, 20] * W) / cEvaluations * 2 * L
        )
        self.assertAlmostEqual(normsquared_start, 1, delta=0.05)
        self.assertAlmostEqual(normsquared_1, 1, delta=0.05)
        self.assertAlmostEqual(normsquared_end, 1, delta=0.05)

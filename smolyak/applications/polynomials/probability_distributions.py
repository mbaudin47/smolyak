...
class ProbabilityDistribution(AbstractProbabilityDistribution):
    def __init__(self, measure="u", interval=None):
        ...
        if measure == "h":
            ...
            self.interval = (
                None,
                float(interval),
            )  # replace None by center in the future
        else:
    ...
    def lebesgue_density(self, X):
        if self.measure == "u":
            ...
            return 1 / (
                np.pi * np.sqrt((X - self.interval[0]) * (self.interval[1] - X))
            )
        elif self.measure == "h":
            return np.exp(-(X ** 2.0) / (2.0 * self.interval[1])) / np.sqrt(
                2 * self.interval[1] * np.pi
            )

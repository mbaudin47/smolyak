...
def arcsine_samples(probability_space, N):
    def univariate_arcsine_samples(N, interval):
        ...
        D = 1.0 / (np.pi * np.sqrt((X - interval[0]) * (interval[1] - X)))
        return (X, D)

    X = np.zeros((N, probability_space.get_c_var()))
    D = np.ones((N, 1))
    for dim in range(X.shape[1]):
        (X_temp, D_temp) = univariate_arcsine_samples(
            N, probability_space.ups[dim].interval
        )
        X[:, [dim]] = X_temp
        D *= D_temp
    return (X, D)
...
def optimal_samples(tensor_polynomial_subspace, N):
    ...
    for i in range(N):
        ...
        for dim in range(tensor_polynomial_subspace.get_c_var()):
            ...
            X[i, dim] = sample_from_polynomial(
                tensor_polynomial_subspace.probability_distribution.ups[dim], degree
            )
...
def sample_from_polynomial(probability_space, pol):
    ...
    if probability_space.measure == "u":
        acceptance_ratio = 1.0 / (4 * np.exp(1))
    elif probability_space.measure == "c":
        acceptance_ratio = 1.0 / (2 * np.exp(1) * (2 + np.sqrt(1.0 / 2)))
    elif probability_space.measure == "h":
        acceptance_ratio = 1.0 / 8 * (pol + 1) ** (-1.0 / 3)
    accept = False
    while not accept:
        if probability_space.measure in ["u", "c"]:
            X_temp = (np.cos(np.pi * np.random.rand(1, 1)) + 1) / 2
            X = probability_space.interval[0] + X_temp * (
                probability_space.interval[1] - probability_space.interval[0]
            )
            dens_prop_X = 1 / (
                np.pi
                * np.sqrt(
                    (X - probability_space.interval[0])
                    * (probability_space.interval[1] - X)
                )
            )

...
def diffusion_coefficient(d):
    ...
    for dd in range(d):
        dd = dd + 1
        line = int(np.ceil(-1.0 / 2 + np.sqrt(1.0 / 4 + 2 * dd) - 1))
        remains = int(dd - (line ** 2 + line) / 2.0)
        if remains > 0:
            s1 = line + 2 - remains
            s2 = remains
        expression += "+"
        ...
        if s1 % 2 == 0:
            expression += "sin({}*pi*x[0])".format(int(s1 / 2.0))
        else:
            expression += "cos({}*pi*x[0])".format(int((s1 + 1) / 2.0))
        if s2 % 2 == 0:
            expression += "*sin({}*pi*x[1])".format(int(s2 / 2.0))
        else:
            expression += "*cos({}*pi*x[1])".format(int((s2 + 1) / 2.0))
    expression += ")"
    return expression

import numpy as np
from smolyak.applications.particle_systems.particle_approximations import (
...
def univariate_integral(particles, phi, h, shift):
    ...
    return 1.0 / N * np.sum(phi_particles) + h
    ...
    for __ in range(N_steps):
        ...
        for (i, particle) in enumerate(particles):
            for (j, particle_2) in enumerate(particles):
                if i != j:
                    new_particles[i] = new_particles[i] + 1.0 / N_particles * h * (
                        particle_2 - particle
                    ) * np.power(np.absolute(particle_2 - particle), 3)
        particles = new_particles
    phi_particles = phi(new_particles)
    return 1.0 / N_particles * np.sum(phi_particles)


def univariate_integral_approximation(
    N_particles,
    h,
    invcdf=None,
    pdf=None,

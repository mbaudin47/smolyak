...
def update_control(control, particle_history, costate_history, alpha, potential, rho):
    ...
    new_control[0] = control[0] + rho * (
        -alpha / control[0] ** 2
        - 1
        / N_particles
        * np.sum(costate_history[:, :, -1] * particle_history[:, :, 0])
    )
    new_control[1:] = control[1:] + 15 * rho * (
        -1 / N_particles * np.sum(costate_history[:, :, -1], axis=1)
    )
...
def iteration(
    ...
    if random:
        np.random.seed(10)
        normal_discretization = np.random.multivariate_normal(
            np.zeros((d,)), np.eye(d), size=(c_particles,)
        ).T
    else:
        normal_discretization = np.array(
            list(
                itertools.product(
                    *(
                        [scipy.stats.norm.ppf(np.linspace(0, 1, c_particles + 2)[1:-1])]
                        * d
                    )
                )
            )
    ...
    for __ in range(c_iter):
        control = new_control
        init_particles = (
            np.tile(control[1:, None], (1, N_particles))
            + control[0] * normal_discretization
        )
        particle_history = forward_stepping(init_particles, weights, h, potential)
        costate_history = backward_stepping(
            -particle_history[:, :, -1], weights, h, d_potential
        )
        new_control = update_control(
            control, particle_history, costate_history, alpha, potential, rho
        )
        # print(new_control)
    return control
...
if __name__ == "__main__":
    ...
    def d_potential(p1, p2, power, k):
        return (-1) ** k * (
            np.linalg.norm(p2 - p1) ** (power - 1)
            + (power - 1)
            * np.power(p2 - p1, 2)
            * np.linalg.norm(p2 - p1) ** (power - 3)
        )
    ...
    def total_d_potential(p1, p2, k):
        return -d_potential(
            p1, p2, power_reject, k
        )  # +d_potential(p1,p2,power_attract,k)
